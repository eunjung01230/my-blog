...
이 블로그 저장소를 보면 `one`, `two`, `nav`, `week1`처럼 브랜치를 나눠서 글을 쓰고 PR로 main에 합쳐온 기록이 있다. 그런데 정작 "왜 브랜치를 나눠서 글을 쓰는지", "합칠 때 무슨 일이 생길 수 있는지"는 명확히 정리한 적이 없어서 오늘 이 부분을 짚어보기로 했다.
